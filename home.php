...
require_once(XRMS_ROOT."cases/sidebar.php");
...
$sidebar_top = do_hook_function('private_sidebar_top', $sidebar_top);
...
$content_bottom = do_hook_function('private_content_bottom', $content_bottom);
...
add_audit_item($con, $session_user_id, 'viewed', 'worksheet', '', 3);
...
table.widget .widget_label {
    background-color: #4A97DB; /* Principal color setting */
    color: #ffffff;
}

.widget_content_large { font-size: 12px; }
...
<div id="Main">
    <div id="Content">
        ...
        <?php /*if ((!check_user_role(false, $_SESSION['session_user_id'], 'Sales'))
        	&& (!check_user_role(false, $_SESSION['session_user_id'], 'Service'))) {
        		$show_more=1;
...
$show_more=0; ?>
		...
		<?php 
			if ($show_more) {echo $activities_widget['content'];}
			//echo $activities_widget['sidebar']; 
			if ($show_more) {echo $activities_widget['js'];}
		?>
		</form>

        <!-- Body Hook Return (e.g.) Non-Uploaded Files //-->
        <?php   if ($show_more) {echo $private_body_rows;} ?>
...
/*if (($session_user_id == 2) || ($session_user_id == 3) || ($session_user_id == 14)
 || ($session_user_id == 8) || ($session_user_id == 27) || ($session_user_id == 38) || ($session_user_id == 39) || ($session_user_id == 36) || ($session_user_id == 63) || ($session_user_id == 40) 
 || ($session_user_id == 52) || ($session_user_id == 53) || ($session_user_id == 50) || ($session_user_id == 20) || ($session_user_id == 5) || ($session_user_id == 62) || ($session_user_id == 30)
 || ($session_user_id == 91) || ($session_user_id == 82)) {
*/	
?>
        <div width="80%">
        <iframe name="tabIframe2" src="<?php echo XRMS_FOLDER_PATH;?>/plugins/reports/dashboard_stats.php" marginheight="0" marginwidth="8" frameborder="0" scrolling="no" 
        style="height:170px; width: 100%; border-top: 0px solid #ccc; border-right: 0px; border-left: 0px; border-bottom: 0px solid #ccc">
        </iframe>
        </div>
        
 <? /*}*/ ?>               
        
<!-- ######################################### NEW DASHBOARD 06/03/08 ############################################# -->

<? 

####################################################################################################################
#
#               LOAD DASHBOARD FOR AUTHORIZED USERS
#
####################################################################################################################
...
?>
        ...
        <div width="80%">
        <iframe name="tabIframe1" src="<?php echo XRMS_FOLDER_PATH;?>/plugins/reports/dashboard_new_patients.php" marginheight="0" marginwidth="8" frameborder="0" scrolling="no" 
        style="height:700px; width: 100%; border-top: 0px solid #ccc; border-right: 0px; border-left: 0px; border-bottom: 0px solid #ccc">
        </iframe>
        </div>
...
-->
            ...
            <!-- opportunities //-->
            <?//php  if ($show_opp_sidebar) {echo $opportunity_rows;} ?>

            <!-- cases //-->
            <?//php  if ($show_cases_sidebar) {echo $case_rows;} ?>

            <!-- files //-->
            <?//php  if ($show_more) {echo $file_rows;} ?>

            <!-- notes //-->
            <?//php  if ($show_notes_sidebar) {echo $note_rows;} ?>

        <!-- sidebar plugins //-->
        <?php if (1) {echo $sidebar_rows;} ?>

        
            
    <table class=widget cellspacing=1 height=50>
    <tr>
    <td class=widget_header style="background-color: #4A97DB">
    Legend
		</td>
		<tr>
		<td style="padding: 10px">
		<span style="font-size: 11px">
		<p><b>M100 Program</b><br>
		   F1 - Declined due to bad SSN<br>
		   F2 - Declined due to FICO<br>
		   F3 - Declined due to few trade lines<br>
		   F4 - Declined due to other reasons
		</p>
		   
		<!--<a href="/xrms/changelog/index.php" target="tabIframe1">Updates</a>-->
		<!--Please refer to the change log for technical updates and announcements that impact the XRMS system. For those that
		are using Internet Explorer browser, XRMS is not compatible with IE please use <a href="http://www.mozilla.com/en-US/firefox/">Firefox</a>.-->
